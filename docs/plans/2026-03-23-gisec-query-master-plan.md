# GISEC Query Alpha Master Implementation Plan
...
**Architecture:** `query-alpha` is intentionally narrow. It freezes the current repository as `v1.5 legacy`, re-enters the new object-first line through formal `gisec` `query_*` modules, and proves one minimal hypothesis first: `UQ-s/UQ-m` query-only object-first segmentation is stronger and more stable than the current fragment-first pipeline. Only after that mainline is validated do `reference` and `graph` re-enter as separate later phases, because they remain mandatory paper modules but cannot be allowed to define the backbone before the backbone itself is strong.

**Tech Stack:** Python 3.13, PyTorch 2.10, current YAML config stack, COCO evaluation, existing logging/export contract, formal `gisec` `query_*` modules for semantic isolation.

---

## Summary
- Freeze the current implementation as `GISEC v1.5 legacy`.
- Build `query-alpha` in a hard-isolated code path, not by incrementally mutating the current fragment-first core classes.
- formal `gisec` `query_*` modules must not import legacy `VariantSpec`, legacy `graph_utils.py`, or the old fragment-first runtime as the default query core.
...
- Introduce formal `gisec` `query_*` modules with separate configs, runners, and tests where needed.
- Keep shared infra only where semantics are identical:
  - dataset protocol
  - COCO export contract
  - run-summary artifact naming

### Phase 1: query-alpha Query-Only Mainline
- Implement only:
  - `UQ-s`
  - `UQ-m`
...
- New query model family naming:
  - `UQ-s`
  - `UQ-m`
  - later `UR-*`, `UG-*`, `UA-*`
- New `query-alpha` model outputs:
  - `fg_logits`
  - `boundary_logits`
  - `core_heatmap`
  - `ownership_offsets`
...
## Deliverables
- One master plan:
  - `docs/plans/2026-03-23-gisec-query-master-plan.md`
- Five subplans:
  - `docs/plans/2026-03-23-01-gisec-query-freeze-and-separation.md`
  - `docs/plans/2026-03-23-02-gisec-query-uq-backbone.md`
  - `docs/plans/2026-03-23-03-gisec-query-object-proposal-and-training.md`
  - `docs/plans/2026-03-23-04-gisec-query-eval-ladder.md`
  - `docs/plans/2026-03-23-05-gisec-query-reference-graph-reentry.md`

## Acceptance
- `v1.5 legacy` and `query-alpha` are separated by code path, config path, and document naming.
...
- Confirm every `query-alpha` subplan refers only to `UQ-s/UQ-m` in the first execution phase.
