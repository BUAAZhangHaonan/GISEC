# AffiniGraph Diagnostics Plan

## Goal
Turn the Stage 1 matrix into a mechanism story by documenting why the GraphRefiner works, where it fails, and what features the reference bank must supply.

## Scope
- Define overlays, fragment graphs, and threshold sensitivity experiments for each key variant.
- Assess reference bank quality sensitivity so the repo can say how `shape_stats`, `rgb similarity`, or `depth cues` influence outcomes.
...
- The diagnostics report clearly states which reference bank attributes help (or hurt) and includes a short verbal summary.
