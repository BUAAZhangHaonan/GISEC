# AffiniGraph Graph Matrix Plan

## Goal
Deliver the Stage 1 matrix that proves a reference-guided graph refiner beats the cleaned B0 baseline under the fixed `0831_1K / 1024 / 20 epochs` regime.
...
## Key Changes
- Create a `scripts/experiments/run_0831_1k_20ep_1024_affinigraph_all.sh` that loops through the new CLI and writes logs per variant.
