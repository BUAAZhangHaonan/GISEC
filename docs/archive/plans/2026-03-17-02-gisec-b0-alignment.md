 ...
 ## Scope
 - Introduce `gisec.config.variants.VariantSpec` to codify B0–G5 toggles.
 ...
 ## Key Changes
 - Replace the `VARIANT_FLAGS` table with declarative spec entries that expose `use_shape_stats`, `use_rgb_prototype_similarity`, `use_depth_prototype_similarity`, and `use_learned_edge_scorer`.
