...
from gisec.config.variants import gisec_variant_names, get_gisec_variant_spec
...
def _validate_checkpoint_variant(
    *,
    ...
    raise RuntimeError(
        ...
        f"but the requested GISEC variant is {expected_variant}."
    )
...
def _move_gisec_tensor_to_device(tensor: Any, device: torch.device, *, non_blocking: bool) -> Any:
    ...
    *,
    ...
    for sample in samples:
        ...
        if masks is None or labels is None:
            ...
            continue
        mask_labels.append(_move_gisec_tensor_to_device(masks.float(), device, non_blocking=non_blocking))
        class_labels.append(_move_gisec_tensor_to_device(labels.long(), device, non_blocking=non_blocking))
