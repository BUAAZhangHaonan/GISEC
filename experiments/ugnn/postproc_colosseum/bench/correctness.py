...
def main():
    ...
    args = ap.parse_args()
    metas = {
        int(m["image_id"]): m for m in json.loads((DUMPS / "metajs.json").read_text())
    }
    ...
    for dt in (dt_ref, dt_usr):
        ...
        ev.params.imgIds = PROBE
        ev.evaluate()
        ev.accumulate()
        ev.summarize()
        aps.append(float(ev.stats[0]))
    ...
    c4 = det_ok
    print(
        f"C1 count: zero-dev frac={np.mean([d == 0 for d in devs]):.4f} max_dev={max(devs)} -> {'PASS' if c1 else 'FAIL'}"
    )
    print(f"C2 IoU:   mean={np.mean(ious):.6f} -> {'PASS' if c2 else 'FAIL'}")
    print(
        f"C3 AP:    ref={aps[0]:.4f} usr={aps[1]:.4f} |d|={abs(aps[0] - aps[1]):.5f} -> {'PASS' if c3 else 'FAIL'}"
    )
