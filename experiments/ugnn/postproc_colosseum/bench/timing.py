...
def main():
    ...
    # warmup 10
    for iid in ids[:10]:
        d = np.load(HERE/'data/dumps'/f'{iid}.npz'); m = metas[iid]
        fn(iid, d['sem'].astype(np.uint8), d['hm'].astype(np.float32),
           d['off'].astype(np.float32), d['depth'], m['height'], m['width'])
    times = []
    for iid in ids[:args.n]:
        d = np.load(HERE/'data/dumps'/f'{iid}.npz'); m = metas[iid]
        args_t = (iid, d['sem'].astype(np.uint8), d['hm'].astype(np.float32),
                  d['off'].astype(np.float32), d['depth'], m['height'], m['width'])
        t0 = time.perf_counter(); fn(*args_t); times.append((time.perf_counter()-t0)*1e3)
    print(f'module={args.module} n={len(times)} median={statistics.median(times):.2f} ms/img '
