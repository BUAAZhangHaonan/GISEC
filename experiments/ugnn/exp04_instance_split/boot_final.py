...
from eval_pipeline import (  # noqa: E402
    load_split, predict_semantic, scene_bootstrap, score_area,
)
...
model.load_state_dict(
    torch.load(HERE.parent / "exp03_unet_dense" / "runs" / "best.pth",
               map_location="cpu"))
model.cuda()
...
for it, insts in zip(items, per_img, strict=True):
    ...
    results.extend(masks_to_coco_results(
        image_id=it["image_id"], masks=[m for m, _ in insts],
        scores=scores, category_id=1))
