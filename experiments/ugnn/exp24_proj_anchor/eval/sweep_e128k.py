"""E25 (128K long-train) judged sweep: 500-image SEM_THR grid.

PRE-REGISTERED candidate set (written before any scoring, 2026-09-01):
the 16 evenly-spaced 8K-step EMA snapshots (k08..k128) + the final 10
per-epoch EMA checkpoints (ep70..ep79); baseline row = frozen E24 winner
(ep13 forward cache, never re-run). Same frozen 500 images / THRS grid /
legacy decode / maxDets as sweep_e24.py.
...
Tags (multiplicity shared downstream in crossfit_e24.py):
  e20            exp20 runs/best.pth via the existing decode_fix cache
                 (reproduction gate vs sweep_decode.json legacy rows)
  ep13..ep19     exp24 runs/ema_ep{13,15,17,18,19}.pth second-half EMA
                 snapshots (best.pth == final-epoch EMA; train_log EMA
                 monotone up through ep19)

Stages:
  A (GPU) forward each e24 tag over the 500 images -> _cache_fwd/{tag}/
  B (CPU) decode {tag} x THRS grid -> AP table + seed precision

Run: systemd-run --user -p MemoryMax=32G -p CPUQuota=3200%
Output: eval/sweep_e24.json
"""
...
ALIGN_TOL = 5e-6
SNAP_TAGS = ['k08', 'k16', 'k24', 'k32', 'k40', 'k48', 'k56', 'k64', 'k72', 'k80', 'k88', 'k96', 'k104', 'k112', 'k120', 'k128']
EMA_TAGS = ['ep70', 'ep71', 'ep72', 'ep73', 'ep74', 'ep75', 'ep76', 'ep77', 'ep78', 'ep79']
...
def _ckpt_path(t):
    if t.startswith("k"):
        step = int(t[1:]) * 1000
        return RUNS128K / f"snap_{step:07d}.pth"
    return RUNS128K / f"ema_{t}.pth"

CKPT = {t: _ckpt_path(t) for t in CAND_TAGS}
FWD = {
...
def stage_b(tags: list[str]) -> None:
    ...
    align = None
    HIST = EVAL / "sweep_e24.json"
    if "e24" in tags and HIST.exists():
        hist = json.loads(HIST.read_text())["scores"]
        diffs = {
            str(x): scores[f"e24@{x}"]["AP"] - hist[f"ep13@{x}"]["AP"] for x in THRS
        }
        align = {"max_abs_diff": max(abs(v) for v in diffs.values()), "per_thr": diffs}
        print(f"e24-vs-sweep_e24 alignment max|dAP|={align['max_abs_diff']:.2e}")
        if align["max_abs_diff"] > ALIGN_TOL:
            raise SystemExit("A2 FAIL: e24 rows deviate from sweep_e24.json")
    ...
    (EVAL / "sweep_e128k.json").write_text(
        json.dumps(
            {
                ...
                "scores": scores,
                "best_thr": best,
                "e24_alignment_vs_sweep_e24": align,
                "seed_precision": seed,
            },
            indent=2,
        )
    )
    print("wrote sweep_e128k.json", flush=True)
