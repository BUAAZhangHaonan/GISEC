...
PREREG_E24_FULL = 0.861132
N_BOOT = 2000
SEED = 0
SMALL_AREA = 1024  # COCO areaRng small upper bound (32^2), == pkl digitize

FWD = {
    "e128k": None,  # set from --tag: HERE/_cache_fwd128k/{tag}
    "e24": HERE / "_cache_fwd" / "ep13",
}
_THR = {"e128k": None, "e24": 0.95}
...
def stage_a(tag: str) -> list[dict]:
    ...
    ckpt_path = (
        RUNS128K / f"snap_{int(tag[1:]) * 1000:07d}.pth"
        if tag.startswith("k")
        else RUNS128K / f"ema_{tag}.pth"
    )
    ckpt = torch.load(ckpt_path, map_location="cpu", weights_only=True)
    model = SeedNet()
    model.load_state_dict(ckpt["model"])  # strict: arch-parity gate
    model.cuda().eval()
    print(f"[{tag}] loaded {ckpt_path.name} step={ckpt.get('step')}", flush=True)
...
def stage_b(img_ids: list[int]) -> dict:
    buckets: dict = {}
    jobs = [(m, i, _THR[m]) for m in ("e128k", "e24") for i in img_ids]
    t0 = time.perf_counter()
...
def stage_c(buckets: dict, metas: list[dict], tag: str, thr: float) -> dict:
    ...
    for m in ("e128k", "e24"):
        ...
        accs[m] = acc
    out["full"] = full
    out["gate_g1_e24_prereg"] = {
        "point": full["e24"]["AP"],
        "prereg": PREREG_E24_FULL,
        "abs_diff": abs(full["e24"]["AP"] - PREREG_E24_FULL),
    }
    if abs(full["e24"]["AP"] - PREREG_E24_FULL) > 5e-4:
        raise SystemExit(f"G1 FAIL: e24 full AP {full['e24']['AP']:.7f}")
    print("G1/G2 pass; full:", json.dumps(full), flush=True)
    out["canonical_ci_e24"] = paired_scene_bootstrap(
        accs["e128k"], accs["e24"], resampler, N_BOOT, SEED
    )
    ...
    for name, ids in (("small_imgs", small_img_ids), ("other_imgs", other_img_ids)):
        ...
        shared = paired_scene_bootstrap(
            ApWeighted(coco_gt, coco_gt.loadRes(list(buckets["e128k"])), ids, "segm"),
        ...
        d = shared["delta"]
        print(
            f"{name}: e24 {entry['e128k']['AP']:.6f} vs e20 {entry['e24']['AP']:.6f} "
    ...
    acc: dict = {(m, c): [] for m in ("e128k", "e24") for c in ("cent", "proj")}
    with mp.get_context("fork").Pool(16) as pool:
    ...
    (EVAL / "eval_full_e128k.json").write_text(json.dumps(out, indent=2))
