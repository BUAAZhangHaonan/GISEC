"""E22: band weight extended to the dice term (fork of E20, single change).

Only change vs E20: dice_loss now takes the same w = 1 + 7*band weight
map the BCE uses (BCE call verbatim E20, BAND_GAIN=7.0 unchanged):
weighted soft dice with per-sample reduction and smooth=1, loss = 1 -
dice_w. Hypothesis: the seam band is under-weighted in the dice term
too; weighting both terms sharpens seam logits without touching the
threshold sweep. E20 header follows.

E20 was: band weight x8 (fork of E17 train_band_ema.py, single-variable).

E20 change vs E17: BAND_GAIN 3.0 -> 7.0, i.e. BCE weight 1 + 7*band
(band interior x8, was x4). Hypothesis: sharper seam logits make the mix
elevation knife cut better; risk of over-sharpness is absorbed by the
SEM_THR sweep (grid up to 0.995). No hflip (E19 ruled out). Everything
else verbatim E17: EMA 0.999, cosine 20ep, best by EMA mIoU, 16.851M
params, lock protocol, {'model','step'} checkpoints.

Original E17 header below.

E15 forensics: misses are dense same-depth contacts welded into one sem
blob by union supervision (cov 0.998, local precision 0.35); the seam
gradient is already the operative knife signal (E12 +2.2pt). Two training
changes, zero new heads / zero new params (total stays 16.851M):

  1. band-weighted BCE: weight = 1 + 3*band where band is the union of
     per-instance dilate(m,3x3) & ~erode(m,3x3) rims (both sides of every
     seam + outer contours). Band pixels weigh x4 in the BCE term only;
     Dice / focal / offset terms unchanged. Bands are precomputed by
     build_band_records.py into gt_records/{split}_band.dat (packbits,
     aligned with gt_records items order) because items.pkl carries no
     per-instance masks and decoding raw annotations in workers is the
     COW hazard E9b removed.
  2. EMA (decay 0.999, updated every opt step) over the full state dict
     (float tensors averaged, non-float copied). Val evaluates raw AND
     EMA weights (swap in/out); best.pth stores the EMA state dict;
     best is selected by EMA mIoU.
...
--lock-file: path touched-removed around the run (GPU priority gate vs
the baselines16m chain); removed in a finally + atexit hook.
"""
...
def dice_loss(logits, targets, weight):
    """E22: band-weighted soft dice (single change vs E20 dice term).

    Same per-sample reduction and smooth=1 constant as the E20 dice
    (which already used +1 smoothing); only the pixel sums are weighted
    by w = 1 + BAND_GAIN*band, the identical weight map the BCE uses:
        dice_w = (2*sum(w*p*t) + s) / (sum(w*p) + sum(w*t) + s),  s = 1
    loss = 1 - dice_w, per-sample mean over the batch.
    """
    p = torch.sigmoid(logits)
    inter = (weight * p * targets).sum(dim=(1, 2, 3))
    union = (weight * p).sum(dim=(1, 2, 3)) + (weight * targets).sum(dim=(1, 2, 3))
    ...
    try:
        for epoch in range(start_step // len(dl), args.epochs):
            ...
            for step, (x, y_sem, y_seed, y_band) in enumerate(dl):
                ...
                l_dice = dice_loss(sem, y_sem[:, None], w)
