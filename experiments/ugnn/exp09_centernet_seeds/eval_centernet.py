...
offset) instead of peak_local_max on a 1024 heatmap. Watershed
elevation (grad depth), merge postprocess, scoring, bootstrap are
byte-identical to E8c.
...
from eval_scale import (DATA, HM_THR, rss_gb,  # noqa: E402
                        gt_center_markers, gt_centers, seed_precision,
                        SplitStats, to_results, scene_key)
...
def _worker_one(payload):
    ...
    coords_by_tag = {
        ...
        "centernet": _cn_markers(hm, off) if hm is not None
...
def scene_bootstrap_fast(metas, results, n_boot=100, seed=0):
    ...
    with mp.get_context("fork").Pool(
            ...
            initargs=(coco_gt, coco_dt, scenes)) as pool:
        rows = pool.map(_boot_one, draws, chunksize=1)
    out = {"n_scenes": len(scenes), "n_boot": n_boot}
    for name, vals in (("segm", [r[0] for r in rows]),
                       ("bbox", [r[1] for r in rows])):
