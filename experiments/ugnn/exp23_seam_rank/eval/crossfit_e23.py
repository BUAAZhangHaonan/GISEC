"""E23 winner selection: scene-disjoint (epoch, thr) cross-fitting.
...
lib/scene_boot.cross_fit_threshold exactly:
  ...
  joint        each draw additionally re-picks the winning EPOCH on
               the calibration replicate (argmax over E23 tags at
...
def joint_cross_fit(accs_by_thr, resampler, n_boot, seed):
    ...
    per draw the winning E23 tag is re-picked on the calibration
    ...
    e23_tags = [t for t in accs_by_thr if t != BASE]
    ...
    for d in range(n_boot):
        ...
        win = max(e23_tags, key=lambda v: calib_ap[v])
...
def main() -> None:
    ...
    for t in TAGS:
        for thr in THRS:
            ...
            if t == BASE and thr == 0.9:
                gate_a2 = {"point": point, "prereg": PREREG_E20_090}
                if abs(point - PREREG_E20_090) > ALIGN_TOL:
                    raise SystemExit(f"A2 FAIL: e20@0.9 = {point:.7f}")
    ...
    (EVAL / "crossfit_e23.json").write_text(json.dumps(out, indent=2))
    ...
    print("wrote crossfit_e23.json", flush=True)
