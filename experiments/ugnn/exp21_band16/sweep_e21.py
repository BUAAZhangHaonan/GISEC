"""E21 judgment sweep: band x16 ckpt vs the E20 canonical row (thr 0.9).

Pre-registered (exp21_band16/RESULT.md header):
  PASS(1) = 500-img best SEM_THR over {0.7,0.8,0.85,0.9,0.95,0.97}
            (extend to 0.6 if 0.7 stays at the edge), paired dAP vs
            the E20 row (thr 0.9, AP 0.84713) with dAP > 0 and CI
            LOWER BOUND > 0.
  PASS(2) = full 3276 fast FINAL > 0.84880 (run separately on win).
  Guardrail: seed dist median < 8 px; train-log best mIoU >= 0.997.

Stage A (GPU): forward exp21 best.pth through SeedNetE10 strict load
(the strict load is the arch-parity smoke check), cache
sem_logit/hm/off/depth for the first 500 val images.

Stage B (CPU): both caches (E21 here, E20 from exp20_band8/_cache_fwd)
...
scene bootstrap (100 draws, seed 0, same machinery as sweep_e20).
"""
...
THRS = [0.7, 0.8, 0.85, 0.9, 0.95, 0.97]
E20_THR = 0.9
...
def stage_a(ckpt_path: str) -> None:
    ...
    metas = metas[:N_IMG]
    FWD_E21.mkdir(parents=True, exist_ok=True)
    ...
    for i, meta in enumerate(metas):
        npz = FWD_E21 / f"{meta['image_id']}.npz"
        if npz.exists():
            continue
...
def _one_image(payload):
    ...
    z = np.load((FWD_E21 if tag == "e21" else FWD_E20) / f"{image_id}.npz")
...
def _seed_pairs(metas):
    ...
    for meta in metas:
        ...
        z = np.load(FWD_E21 / f"{meta['image_id']}.npz")
...
def _logit_stats(img_ids):
    out = {}
    for tag, fwd in (("e21", FWD_E21), ("e20", FWD_E20)):
        means, band = [], []
        for i in img_ids:
...
def stage_b() -> None:
    ...
    jobs = [("e21", i, t) for t in THRS for i in img_ids]
    ...
    def best_of(thrs):
        return max(thrs, key=lambda t: scores[f"e21@{t}"]["AP"])

    best_thr = best_of(THRS)
    edge = best_thr == THRS[0]
    if edge:  # guardrail from prereg: 0.7 stays at the edge -> extend 0.6
        extra = [0.6]
        jobs2 = [("e21", i, t) for t in extra for i in img_ids]
        with mp.get_context("fork").Pool(16) as pool:
    ...
    # paired scene bootstrap: e21@best vs e20@0.9 (same draws as sweep_e20)
    scenes = {}
    for m in metas:
    ...
    dts = {
        "e21_best": coco_gt.loadRes(buckets[("e21", best_thr)]),
    ...
    for (v, _d), r in zip(bjobs, rows, strict=True):
        boot[v].append(r)
    d = np.array([r["AP"] for r in boot["e21_best"]]) - np.array(
    ...
    out = {
        ...
        "paired_e21_best_vs_e20_090": paired,
        "seed_precision": seed,
        "logit_stats": _logit_stats(img_ids),
        "verdict": verdict,
        "rule": "PASS1 iff best-thr paired dAP>0 & CI LOWER bound>0 vs E20@0.9 (0.84713); guardrail seed median <8px, mIoU>=0.997 (train_log best EMA 0.99863)",
    }
    (HERE / "sweep_e21.json").write_text(json.dumps(out, indent=2))
