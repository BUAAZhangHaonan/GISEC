...
log_stage_summary() {
  ...
  runner_log "${MODE}" "${root_log}" "[active-mainline] stage=${stage_name}"
  ...
  runner_log "${MODE}" "${root_log}" "[active-mainline] next_artifact=${next_artifact}"
}
...
run_stage_command() {
  ...
  runner_log "${MODE}" "${root_log}" "[active-mainline] completed_dir=${stage_dir}"
}
...
for index in "${!ACTIVE_STAGES[@]}"; do
  ...
  if train_complete "${train_dir}"; then
    runner_log "${MODE}" "${ROOT_LOG}" "[active-mainline] SKIP train ${stage_name}"
  else
    train_args=(
      -m gisec.cli.train
  ...
  if eval_complete "${eval_dir}"; then
    runner_log "${MODE}" "${ROOT_LOG}" "[active-mainline] SKIP eval ${stage_name}"
  else
    eval_args=(
      -m gisec.cli.eval
      --dataset-root "${DATASET_ROOT}"
