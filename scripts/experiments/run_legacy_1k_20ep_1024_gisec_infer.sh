...
PROTOTYPE_ROOT=""
OUTPUT_ROOT="${REPO_ROOT}/output/experiments/gisec_infer_0831"
MODE="run"
VARIANT="G5"
...
runner_log "${MODE}" "${RUN_LOG}" "[gisec-infer-0831] mode=${MODE}"
runner_log "${MODE}" "${RUN_LOG}" "[gisec-infer-0831] variant=${VARIANT}"
runner_log "${MODE}" "${RUN_LOG}" "[gisec-infer-0831] dataset_root=${DATASET_ROOT}"
runner_log "${MODE}" "${RUN_LOG}" "[gisec-infer-0831] prototype_root=${PROTOTYPE_ROOT}"
runner_log "${MODE}" "${RUN_LOG}" "[gisec-infer-0831] checkpoint=${CHECKPOINT}"
