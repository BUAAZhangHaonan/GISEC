...
if [[ -n "${CHECKPOINT}" ]]; then
  MASK2FORMER_CHECKPOINT="${CHECKPOINT}"
fi
...
runner_log "${MODE}" "${RUN_LOG}" "[rgb-weekend] variant=${VARIANT}"
runner_log "${MODE}" "${RUN_LOG}" "[rgb-weekend] infer_cmd=python -m gisec.cli.infer_legacy --variant ${VARIANT}"
