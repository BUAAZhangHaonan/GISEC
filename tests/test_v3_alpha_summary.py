...
def _write_run(
    ...
    *,
    ...
    if use_graph_rescue is not None:
        payload["use_graph_rescue"] = use_graph_rescue
...
def test_v3_alpha_summary_script_rejects_reference_or_graph_contamination(tmp_path: Path) -> None:
    repo_root = Path(__file__).resolve().parents[1]
    suite_root = tmp_path / "suite"
    output_json = tmp_path / "alpha_summary.json"
    output_md = tmp_path / "alpha_summary.md"
    ...
    contaminated = json.loads((suite_root / "uq_s" / "run_summary.json").read_text(encoding="utf-8"))
    contaminated["use_reference"] = True
    (suite_root / "uq_s" / "run_summary.json").write_text(json.dumps(contaminated), encoding="utf-8")
...
def test_v3_alpha_summary_script_allows_legacy_row_without_alpha_module_flags(tmp_path: Path) -> None:
    repo_root = Path(__file__).resolve().parents[1]
    suite_root = tmp_path / "suite"
    output_json = tmp_path / "alpha_summary.json"
    output_md = tmp_path / "alpha_summary.md"
...
def test_v3_alpha_summary_script_requires_alpha_module_flags_for_uq_rows(tmp_path: Path) -> None:
    repo_root = Path(__file__).resolve().parents[1]
    suite_root = tmp_path / "suite"
    output_json = tmp_path / "alpha_summary.json"
    output_md = tmp_path / "alpha_summary.md"
