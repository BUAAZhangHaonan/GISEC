...
def test_query_docs_keep_ur_ug_ua_documented_but_not_alpha_activated() -> None:
    repo_root = Path(__file__).resolve().parents[1]
    ladder_text = (repo_root / "docs" / "experiments" / "gisec-query-ladder.md").read_text(encoding="utf-8")
    short_run_text = (repo_root / "docs" / "experiments" / "gisec-query-short-run-protocol.md").read_text(encoding="utf-8")
    full_entry_text = (repo_root / "docs" / "experiments" / "gisec-query-full-run-entry.md").read_text(encoding="utf-8")
