...
from baseline.yolov8_seg.adapter import export_yolov8_seg_dataset
...
def test_yolov8_adapter_exports_dataset_yaml_and_labels(tmp_path: Path) -> None:
    ...
    exported = export_yolov8_seg_dataset(dataset_root=str(dataset_root), output_dir=str(export_root))
...
def test_yolov8_seg_rgb_baseline_smoke_exports_shared_artifacts(tmp_path: Path, monkeypatch) -> None:
    dataset_root = tmp_path / "dataset"
    output_root = tmp_path / "out"
    _write_dataset(dataset_root)
    monkeypatch.setattr("baseline.yolov8_seg.train.get_ultralytics_yolo_class", lambda: _FakeYOLO)
