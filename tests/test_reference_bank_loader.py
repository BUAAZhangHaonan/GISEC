...
from affinigraph.datasets.reference_bank import ReferenceBankContractError, load_reference_bank
...
def test_load_reference_bank_compat_mode_backfills_missing_shape_stats(tmp_path: Path) -> None:
    root = tmp_path / "refs"
    ...
    bank = load_reference_bank(root, image_size=64, contract_mode="compat")
...
def test_load_reference_bank_strict_mode_reports_missing_contract_artifacts(tmp_path: Path) -> None:
    root = tmp_path / "refs"
    for name in ["rgb", "depth", "mask", "meta", "camera"]:
        (root / name).mkdir(parents=True)
    rgb = np.zeros((32, 32, 3), dtype=np.uint8)
    ...
    with pytest.raises(ReferenceBankContractError) as exc_info:
        load_reference_bank(root, image_size=64, contract_mode="strict")

    assert "meta/shape_stats.json" in str(exc_info.value)
    assert "meta/preview_contact_sheet.png" in str(exc_info.value)


def test_load_reference_bank_strict_mode_rejects_failed_qa_and_manifest_mismatch(tmp_path: Path) -> None:
    root = tmp_path / "refs"
    for name in ["rgb", "depth", "mask", "meta", "camera"]:
        (root / name).mkdir(parents=True)
    rgb = np.zeros((32, 32, 3), dtype=np.uint8)
    ...
    cv2.imwrite(str(root / "mask" / "view0.png"), np.ones((32, 32), dtype=np.uint8) * 255)
    (root / "camera" / "view0.json").write_text('{"view_id":"view0"}', encoding="utf-8")
    (root / "meta" / "manifest.json").write_text('{"part_name":"demo","views":99,"qa_passed":false}', encoding="utf-8")
