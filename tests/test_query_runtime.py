...
from gisec.engine.query_runtime import count_core_peaks, predict_instance_map


def test_query_count_core_peaks_applies_sigmoid_to_logits() -> None:
...
def test_query_count_core_peaks_consolidates_plateau_into_single_peak() -> None:
...
def test_query_count_core_peaks_can_use_low_confidence_local_maxima_when_signal_is_consistent() -> None:
...
def test_query_predict_instance_map_keeps_single_object_whole() -> None:
...
def test_query_predict_instance_map_splits_supported_two_core_object() -> None:
    fg_logits = torch.full((16, 16), -4.0, dtype=torch.float32)
    fg_logits[3:13, 3:13] = 4.0
    boundary_logits = torch.full((16, 16), -4.0, dtype=torch.float32)
...
def test_query_predict_instance_map_can_split_one_coarse_blob_into_three_instances() -> None:
    ...
    core_logits[10, 5] = 0.5
    core_logits[10, 10] = 0.45
    core_logits[10, 15] = 0.4
