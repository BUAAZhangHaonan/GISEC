...
def test_active_variant_specs_lock_phase_order_and_prototype_requirements() -> None:
    ...
    base_rgb_refine = get_active_variant_spec("base_rgb_1024_refine")
    base_rgb_refine_ref = get_active_variant_spec("base_rgb_1024_refine_ref")
    base_rgb_refine_ref_graph = get_active_variant_spec("base_rgb_1024_refine_ref_graph")
    ...
    assert base_rgb_refine.depth_mode == "rgb"
    assert base_rgb_refine.use_local_refine is True
    assert base_rgb_refine.use_reference_rescue is False
    assert base_rgb_refine.use_graph_rescue is False
    assert base_rgb_refine.requires_prototype_root is False

    assert base_rgb_refine_ref.depth_mode == "rgb"
    assert base_rgb_refine_ref.use_local_refine is True
    assert base_rgb_refine_ref.use_reference_rescue is True
    assert base_rgb_refine_ref.use_graph_rescue is False
    assert base_rgb_refine_ref.requires_prototype_root is True

    assert base_rgb_refine_ref_graph.depth_mode == "rgb"
    assert base_rgb_refine_ref_graph.use_local_refine is True
    assert base_rgb_refine_ref_graph.use_reference_rescue is True
    assert base_rgb_refine_ref_graph.use_graph_rescue is True
    assert base_rgb_refine_ref_graph.requires_prototype_root is True
...
def test_active_surface_files_exist() -> None:
    ...
    assert (repo_root / "configs" / "active" / "base_rgb_1024_refine.yaml").exists()
    assert (repo_root / "configs" / "active" / "base_rgb_1024_refine_ref.yaml").exists()
    assert (repo_root / "configs" / "active" / "base_rgb_1024_refine_ref_graph.yaml").exists()
