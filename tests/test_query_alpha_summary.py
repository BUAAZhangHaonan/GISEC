...
def test_query_alpha_summary_script_rejects_reference_or_graph_contamination(tmp_path: Path) -> None:
    repo_root = Path(__file__).resolve().parents[1]
    suite_root = tmp_path / "suite"
    output_json = tmp_path / "alpha_summary.json"
    output_md = tmp_path / "alpha_summary.md"
...
def test_query_alpha_summary_script_requires_alpha_module_flags_for_uq_rows(tmp_path: Path) -> None:
    repo_root = Path(__file__).resolve().parents[1]
    suite_root = tmp_path / "suite"
    output_json = tmp_path / "alpha_summary.json"
    output_md = tmp_path / "alpha_summary.md"
