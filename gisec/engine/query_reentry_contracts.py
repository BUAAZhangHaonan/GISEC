...
def validate_reference_reentry_contract(contract: ReferenceReentryContract) -> None:
    if str(contract.entry_mode) != "rescue_only":
        raise ValueError("Reference re-entry must remain rescue_only in query-alpha.")
    if bool(contract.allow_coarse_object_modulation):
        raise ValueError("Reference re-entry may not modulate coarse-object formation in query-alpha.")
    if bool(contract.allow_second_entry_path):
        raise ValueError("Reference re-entry may not open a second simultaneous entry path in query-alpha.")
