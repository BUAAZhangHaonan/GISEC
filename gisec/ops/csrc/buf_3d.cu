...
#undef CHECK_BUF
        ...
        if (P > 0) {
            ...
            if ((hasBit(P, 5) && img[img_idx]) || (hasBit(P, 6) && img[img_idx + 1]) || (hasBit(P, 9) && img[img_idx + W]) || (hasBit(P, 10) && img[img_idx + W + 1]))
            ...
            if ((hasBit(P, 17) && img[idx - W]) || (hasBit(P, 18) && img[idx - W + 1]) || (hasBit(P, 33) && img[idx + stepz - W]) || (hasBit(P, 34) && img[idx + stepz - W + 1]))
                union_(label, idx, idx - 2 * W);
            if ((hasBit(P, 19) && img[idx - W + 2]) || (hasBit(P, 35) && img[idx + stepz - W + 2]))
                union_(label, idx, idx - 2 * W + 2);
            if ((hasBit(P, 20) && img[idx - 1]) || (hasBit(P, 24) && img[idx + W - 1]) || (hasBit(P, 36) && img[idx + stepz - 1]) || (hasBit(P, 40) && img[idx + stepz + W - 1]))
...
torch::Tensor connected_componnets_labeling_3d(const torch::Tensor &input) {
    ...
    TORCH_CHECK(input.ndimension() == 3, "input must be a [D, H, W] tensor");
    ...
    TORCH_CHECK((D % 2) == 0, "depth must be even");
