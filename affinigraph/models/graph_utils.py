...
from affinigraph.models.reference_cache import ReferenceCache, cosine_similarity_map
...
def build_graph_batch(
    *,
    ...
    sim_cache = None
    if reference_cache is not None and variant_spec.use_rgb_reference_similarity:
        sim_cache = cosine_similarity_map(feature_map, reference_cache.proto_h.to(feature_map.device))[0, 0]
    for label in labels:
        ...
        ref_depth = 0.0
        if reference_cache is not None and variant_spec.use_depth_reference_similarity:
            proto_d = reference_cache.proto_d.to(feature_map.device)
    ...
    edge_features = []
    edge_targets = []
    shape_stats = reference_cache.shape_stats if reference_cache is not None else {}
