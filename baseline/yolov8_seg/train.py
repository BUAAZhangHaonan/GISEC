...
from baseline.yolov8_seg.adapter import export_yolov8_seg_dataset, get_ultralytics_yolo_class
from baseline.yolov8_seg.eval import evaluate_yolov8_seg_baseline
...
def _count_trainable_params(model: Any) -> int:
    ...
    if inner is None or not hasattr(inner, "parameters"):
        return 0
    return sum(param.numel() for param in inner.parameters() if param.requires_grad)


def train_yolov8_seg_baseline(
    *,
    dataset_root: str,
    output_dir: str,
    image_size: int,
    ...
    model_source: str = "yolov8n-seg.pt",
) -> None:
    artifact_root = Path(output_dir)
    artifact_root.mkdir(parents=True, exist_ok=True)
    dataset_export = export_yolov8_seg_dataset(
